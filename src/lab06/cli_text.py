...
def main():
    ...
    cat_parser.add_argument(
        "-n", action="store_true", dest="n", help="Нумеровать строки"
    )
    ...
    stats_parser.add_argument(
        "--input", required=True, dest="input", help="Путь к входному файлу"
    )
    stats_parser.add_argument(
        "--top", type=int, default=5, help="Сколько слов показать (по умолчанию 5)"
    )
